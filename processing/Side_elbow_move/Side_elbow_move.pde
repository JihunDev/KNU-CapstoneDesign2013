...
if( angles[2] > 0.04 && (angles[1] <= 0.9 && angles[1] >= -0.4 )){
    fill(0,255,0);
    }
    
else if( angles[6] > 0.04 && (angles[5] <= 0.9 && angles[5] >= -0.4 ) ){
    fill(0,255,0);
    }
...
void updateAngles() {
  ...
  kinect.getJointPositionSkeleton(1, SimpleOpenNI.SKEL_NECK, neck);
...
angles[0] = atan2(PVector.sub(rShoulder, lShoulder).z, PVector.sub(rShoulder, lShoulder).x);
  ...
  angles[9] = angle( lShoulder, rShoulder, rElbow);
  angles[10] = angle( lElbow, lShoulder, rShoulder);
